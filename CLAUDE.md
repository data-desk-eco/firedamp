...
The sole attribution mechanism: `agent/run.py` loops through plumes and runs a full research agent per plume — pi headless driving DeepSeek (`deepseek-v4-pro`, needs `DEEPSEEK_API_KEY`) with bash + web tools, no imagery. Results accumulate in `web/data/attributions.json` (committed to git; dist.sh copies it), which the frontend serves directly.
