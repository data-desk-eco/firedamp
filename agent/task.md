...
- `permian` — for permian basin plumes only: texas rrc and new mexico ocd regulatory records (wells with per-lease operator-reported flaring volumes, swr-32 flare permits, gas plants, nm flare/vent incident reports) plus independently observed flare sites from viirs nightfire and sentinel-2. cross-reference: a plume at a flare site with no coincident ir signal points to an unlit flare or venting; reported flaring on the lease names the responsible operator.
